...
class UploadBehavior extends ModelBehavior
{
    ...
    public $defaults = array('rootDir' => null, 'pathMethod' => 'primaryKey', 'path' => '{ROOT}webroot{DS}files{DS}{model}{DS}{field}{DS}', 'fields' => array('dir' => 'dir', 'type' => 'type', 'size' => 'size'), 'mimetypes' => array(), 'extensions' => array(), 'maxSize' => 2097152, 'minSize' => 8, 'maxHeight' => 0, 'minHeight' => 0, 'maxWidth' => 0, 'minWidth' => 0, 'thumbnails' => true, 'thumbnailMethod' => 'imagick', 'thumbnailName' => null, 'thumbnailPath' => null, 'thumbnailPrefixStyle' => true, 'thumbnailQuality' => 75, 'thumbnailSizes' => array(), 'thumbnailType' => false, 'deleteOnUpdate' => false, 'mediaThumbnailType' => 'png', 'saveDir' => true, 'deleteFolderOnDelete' => false, 'keepFilesOnDelete' => false, 'mode' => 0777, 'handleUploadedFileCallback' => null, 'nameCallback' => null,);
    
    protected $_imageMimetypes = array('image/bmp', 'image/gif', 'image/jpeg', 'image/pjpeg', 'image/png', 'image/vnd.microsoft.icon', 'image/x-icon', 'image/x-png',);
    
    protected $_mediaMimetypes = array('application/pdf', 'application/postscript',);
    ...
    public function beforeSave(Model $model, $options = array()) {
        ...
        foreach ($this->settings[$model->alias] as $field => $options) {
            ...
            if ($this->_shouldUpdate($model, $field, $removing)) {
                ...
                if ($removing) {
                    $model->data[$model->alias] = array_merge($model->data[$model->alias], array($field => null, $options['fields']['type'] => null, $options['fields']['size'] => null, $options['fields']['dir'] => null,));
                    
                    $this->_removingOnly[$field] = true;
                    continue;
                }
                
                $model->data[$model->alias][$field] = array($field => null, $options['fields']['type'] => null, $options['fields']['size'] => null,);
            ...
            $this->runtime[$model->alias][$field]['name'] = $this->_retrieveName($model, $field, $this->runtime[$model->alias][$field]['name'], $model->data, array('saveType' => $isUpdating ? 'update' : 'create',));
            
            $model->data[$model->alias] = array_merge($model->data[$model->alias], array($field => $this->runtime[$model->alias][$field]['name'], $options['fields']['type'] => $this->runtime[$model->alias][$field]['type'], $options['fields']['size'] => $this->runtime[$model->alias][$field]['size']));
        }
        return true;
    }
    ...
    protected function _retrieveName(Model $model, $field, $currentName, $data, $options = array()) {
        $options = array_merge(array('rootDir' => $this->settings[$model->alias][$field]['rootDir'], 'saveType' => 'create', 'geometry' => null, 'size' => null, 'thumbnailType' => null, 'thumbnailName' => null, 'thumbnailMethod' => null, 'mediaThumbnailType' => null, 'dir' => null,), $options);
    ...
    protected function _resizeImagick(Model $model, $field, $path, $size, $geometry, $thumbnailPath) {
        ...
        $fileName = str_replace(array('{size}', '{geometry}', '{filename}', '{primaryKey}'), array($size, $geometry, $pathInfo['filename'], $model->id), $this->settings[$model->alias][$field]['thumbnailName']);
    ...
    protected function _exifOrientationTransformations($orientation) {
        $trans = array('flip_vert' => false, 'flip_horz' => false, 'rotate_clockwise' => 0,);
    ...
    protected function _grab(Model $model, $field, $uri) {
        ...
        $model->data[$model->alias][$field] = array('name' => $fileName, 'type' => $headers['Content-Type'], 'tmp_name' => $tmpFile, 'error' => 1, 'size' => (isset($headers['content-length']) ? $headers['Content-Length'] : 0),);
    ...
    protected function _pathThumbnail(Model $model, $field, $params = array()) {
        return str_replace(array('{size}', '{geometry}'), array($params['size'], $params['geometry']), $params['thumbnailPath']);
    }
    ...
    protected function _createThumbnails(Model $model, $field, $path, $thumbnailPath) {
        ...
        if (($isImage || $isMedia) && $createThumbnails && $hasThumbnails) {
            ...
            foreach ($this->settings[$model->alias][$field]['thumbnailSizes'] as $size => $geometry) {
                $thumbnailPathSized = $this->_pathThumbnail($model, $field, compact('geometry', 'size', 'thumbnailPath'));
    ...
    /**
     * Autmatically generates url for existing files
     *
     * @param $model - model to which the behaviour is attached
     * @param $results - array of find results
     * @param $primary - bool - is the search result primary
     * @return array of resutls
     *
     */    
            ...
            foreach ($results as $key => $val) {
                foreach (array_keys($this->settings[$model->alias]) as $field) {
                    if (isset($val[$model->alias][$field])) {
                        $results[$key][$model->alias][$field . "_url"] = DS . 'files' . DS . Inflector::underscore($model->alias) . DS . $field . DS . $val[$model->alias][$model->primaryKey] . DS . $val[$model->alias][$field];
                    }
                }
            }
        }
